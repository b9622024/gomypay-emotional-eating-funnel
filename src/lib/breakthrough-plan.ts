...
export async function completeBreakthroughLevel(accessToken:string,input:{levelNumber:number;userNotes?:string;selectedBranch?:keyof typeof branches;userInputs?:Record<string,unknown>;map?:Record<string,unknown>}){
  ...
  await prisma.$transaction(async tx=>{
    ...
    if(input.levelNumber===7&&input.map){const m=input.map as any,a=structuredAnalysis as any,data={primaryType:state.quiz?.primaryType??"unknown",secondaryType:state.quiz?.secondaryType??null,recommendedRoute:state.profile?.route??"自選路線",primaryCharacterName:state.characters.primary?.characterName??null,primaryOriginalTypeName:state.characters.primary?.originalTypeName??null,secondaryCharacterName:state.characters.secondary?.characterName??null,secondaryOriginalTypeName:state.characters.secondary?.originalTypeName??null,highRiskTime:a.keyRiskMoment||m.highRiskTime||null,highRiskScene:a.keyRiskScene||m.highRiskScene||null,mainEmotion:a.mainTriggerMechanism||m.mainEmotion||null,bodySignal:m.bodySignal||null,selectedBranch:state.progress.selectedBranch||null,nutritionGap:a.biggestNutritionGap||m.nutritionGap||null,highRiskDrinkOrSnack:m.highRiskDrinkOrSnack||null,safeSwapOptions:(m.safeSwapOptions??[]) as Prisma.InputJsonValue,dinnerBackups:(m.dinnerBackups??[]) as Prisma.InputJsonValue,rescuePlan:a.personalRescueMapSummary||m.rescuePlan||null,firstRescueAction:a.firstRescueAction||m.firstRescueAction||null,nextWeekAction:a.nextWeekMission||m.nextWeekAction||null,rewardUnlocked:true,rewardUnlockedAt:new Date(),generatedAt:new Date()};await tx.personalRescueMap.upsert({where:{accessToken},update:data,create:{accessToken,...data}})}
  });
