...
export async function getBreakthroughState(accessToken:string){
  ...
  const quiz=await prisma.quizResult.findUnique({where:{accessToken_quizType:{accessToken,quizType}},select:{primaryType:true,secondaryType:true,createdAt:true}}),profile=recommendation(quiz?.primaryType),primaryCharacter=getCravingCharacter(quiz?.primaryType),secondaryCharacter=getCravingCharacter(quiz?.secondaryType),characterCreated=Boolean(quiz);
