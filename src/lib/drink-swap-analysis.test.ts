...
describe("含糖飲料替換清單 Pro 分析引擎",()=>{
  ...
  it("raises pearl milk tea topping, volume and dependency risks for a large full-sugar order",()=>{const result=buildDrinkAnalysis({drinkId:"pearl-milk-tea",feeling:"咀嚼感",sugarHabit:"全糖",cupSize:"大杯",toppings:["珍珠"],context:"壓力大"});expect("scores" in result&&result.scores.topping).toBe(5);expect("scores" in result&&result.scores.volume).toBe(5);expect("scores" in result&&result.scores.dependency).toBe(5);expect("recommendedVersion" in result&&result.recommendedVersion).toContain("珍珠半份")});
  it("distinguishes sports drink context from empty-stomach stimulation",()=>{const result=buildDrinkAnalysis({drinkId:"sports-drink",feeling:"提神",sugarHabit:"半糖",cupSize:"大杯",toppings:["不加料"],context:"下午提神"});expect("scores" in result&&result.scores.emptyStomach).toBeLessThan(3);expect("scores" in result&&result.scores.sugar).toBeGreaterThanOrEqual(4);expect("scenarioNotes" in result&&result.scenarioNotes.join("")).toContain("沒有大量流汗")});
