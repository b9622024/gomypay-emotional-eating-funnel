...
export type BranchAnalysis={recommendedBranch:BranchKey;secondaryBranch:BranchKey;selectedBranch:BranchKey;scores:Record<BranchKey,BranchScore>;confidence:"low"|"medium"|"high";summary:string;combinedPattern:string;primaryEvidence:string[];trialEvidence:string[];secondaryEvidence:string[];exclusiveAction:string};
...
export function analyzeBranchRoutes(input:{answers?:Record<string,string>;selectedBranch?:string},previous:Record<number,any>={},character:any={}):BranchAnalysis{
 ...
 const primary=branchRoutes[recommendedBranch],secondary=branchRoutes[secondaryBranch],combinedPattern=`${primary.name}＋${secondary.name}疊加型`,primaryEvidence=scores[recommendedBranch].evidence.slice(0,4),trialEvidence=[`8 題試煉中有 ${scores[recommendedBranch].trialHits} 題指向${primary.name}`,`試煉分數：${scores[recommendedBranch].trial} 分`],secondaryEvidence=[...scores[secondaryBranch].evidence,`${scores[secondaryBranch].trialHits} 題試煉指向${secondary.name}`].slice(0,3);
 return {recommendedBranch,secondaryBranch,selectedBranch,scores,confidence,summary:confidence==="low"?"目前部分線索不足。完成 8 題情境試煉後，系統會把前三關與角色結果一起計算。":`目前主要路線偏向${primary.name}，次要線索為${secondary.name}。這不是對錯判定，而是找出最值得先處理的地方。`,combinedPattern,primaryEvidence,trialEvidence,secondaryEvidence,exclusiveAction:branchRoutes[selectedBranch].exclusiveAction};
}
