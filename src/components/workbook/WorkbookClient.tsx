...
function SummaryReport({days,ownsAiAssessment,accessToken,preview}:{days:Record<number,DayState>;ownsAiAssessment:boolean;accessToken:string;preview:boolean}){
  ...
  return <section className="wb-report"><span className="wb-kicker">7 DAY SUMMARY</span><h2>7 天總結報告</h2><p className="wb-disclaimer">以下是依照你的填寫內容整理的自我觀察結果，不是醫療或心理診斷。</p><div className="wb-stats"><article><span>平均飢餓</span><strong>{average(hunger)}</strong><small>/ 10</small></article><article><span>平均壓力</span><strong>{average(stress)}</strong><small>/ 10</small></article><article><span>平均疲憊</span><strong>{average(fatigue)}</strong><small>/ 10</small></article></div><div className="wb-insights"><article><span>最常出現的觸發點</span><strong>{commonTrigger}</strong></article><article><span>含糖飲料是否常出現</span><strong>{sugary?"有出現相關紀錄":"目前不明顯"}</strong></article><article><span>營養缺口是否常出現</span><strong>{nutrition?"有出現相關紀錄":"目前不明顯"}</strong></article></div><div className="wb-pattern"><span>初步模式提示</span><div>{patterns.length?patterns.map(x=><b key={x}>{x}</b>):<b>目前資料不足，持續觀察就好</b>}</div></div><div className="wb-report-cta"><h3>想知道自己更深層的減脂卡關原因？</h3><p>透過 5 分鐘、49 題的測驗填寫，能夠知道現在的脈輪能量狀態以及個人天賦優勢，會有專人一對一解析測驗結果找出問題，並且提供專屬對策方案。</p>{ownsAiAssessment?<a className="btn" href="https://lin.ee/UKTsrwq" target="_blank" rel="noreferrer"><span>我要領取 AI 能量減脂測驗</span><span>→</span></a>:<button className="btn" type="button" disabled={buying||preview} onClick={startAssessment}><span>{preview?"預覽模式不建立訂單":buying?"建立 NT$100 訂單中…":"我要領取 AI 能量減脂測驗"}</span><span>→</span></button>}{!ownsAiAssessment&&!preview&&<small className="wb-assessment-price">未於結帳時加購者，單獨購買價為 NT$100</small>}{purchaseError&&<p className="wb-purchase-error">{purchaseError}</p>}</div></section>;
}
...
export default function WorkbookClient({accessToken,preview=false,ownsAiAssessment=false}:{accessToken:string;preview?:boolean;ownsAiAssessment?:boolean}){
  ...
  useEffect(()=>{if(typeof window!=="undefined"&&new URLSearchParams(window.location.search).get("day")==="1")setSelectedDay(1)},[]);
