...
import { workbookDays,type WorkbookField } from "@/content/workbook";
...
function SummaryReport({days}:{days:Record<number,DayState>}){
  ...
  return <section className="wb-report"><span className="wb-kicker">7 DAY SUMMARY</span><h2>7 天總結報告</h2><p className="wb-disclaimer">以下是依照你的填寫內容整理的自我觀察結果，不是醫療或心理診斷。</p><div className="wb-stats"><article><span>平均飢餓</span><strong>{average(hunger)}</strong><small>/ 10</small></article><article><span>平均壓力</span><strong>{average(stress)}</strong><small>/ 10</small></article><article><span>平均疲憊</span><strong>{average(fatigue)}</strong><small>/ 10</small></article></div><div className="wb-insights"><article><span>最常出現的觸發點</span><strong>{commonTrigger}</strong></article><article><span>含糖飲料是否常出現</span><strong>{sugary?"有出現相關紀錄":"目前不明顯"}</strong></article><article><span>營養缺口是否常出現</span><strong>{nutrition?"有出現相關紀錄":"目前不明顯"}</strong></article></div><div className="wb-pattern"><span>初步模式提示</span><div>{patterns.length?patterns.map(x=><b key={x}>{x}</b>):<b>目前資料不足，持續觀察就好</b>}</div></div><div className="wb-report-cta"><h3>想知道自己更深層的減脂卡關原因？</h3><p>如果你完成 7 天後發現嘴饞和壓力、睡眠、外食、營養缺口有關，可以接著填寫 AI 能量減脂初評，我會協助你判斷下一步怎麼調整。</p><a className="btn" href="#"><span>完成 AI 能量減脂初評</span><span>→</span></a></div></section>;
}

export default function WorkbookClient({accessToken,preview=false}:{accessToken:string;preview?:boolean}){
  ...
  return <div className="workbook-app"><header className="wb-header"><div><Link href={`/access/${accessToken}`}>← 回到工具包</Link><span>可樂吉健康研究所</span></div><div className="wb-progress-copy"><strong>{completed} / 7 天完成</strong><span>{progress}%</span></div><div className="wb-progress"><i style={{width:`${progress}%`}}/></div></header>{!day?<main className="wb-overview"><div className="wb-intro"><span className="wb-kicker">MOBILE WORKBOOK</span><h1>手機互動版<br/>7 天嘴饞止損工作本</h1><p>每天花 5–10 分鐘，直接在手機上打勾、評分與寫下觀察。內容會自動儲存到你的專屬帳戶。</p></div><section className="wb-day-grid">{workbookDays.map(item=>{const state=days[item.day];return <button type="button" className={`wb-day-card ${state?.isCompleted?"completed":""}`} key={item.day} onClick={()=>setSelectedDay(item.day)}><span>DAY {item.day}</span><strong>{item.title}</strong><small>{state?.isCompleted?"✓ 已完成":"開始填寫 →"}</small></button>})}</section>{days[7]?.isCompleted&&<SummaryReport days={days}/>}</main>:<main className="wb-day-page"><button type="button" className="wb-back" onClick={()=>setSelectedDay(null)}>← 回到 7 天總覽</button><div className="wb-day-title"><span>DAY {day.day}</span><h1>{day.title}</h1><p>{day.intro}</p><div className={`wb-save-state ${saveState[day.day]??"idle"}`}>{saveState[day.day]==="saving"?"儲存中…":saveState[day.day]==="saved"?"✓ 已儲存":saveState[day.day]==="error"?"儲存失敗，請重試":"填寫後會自動儲存"}</div></div>{day.sections.map(section=><section className="wb-form-section" key={section.title}><div className="wb-form-heading"><h2>{section.title}</h2>{section.description&&<p>{section.description}</p>}</div>{section.fields.map(field=><FieldControl key={field.key} field={field} value={days[day.day]?.data[field.key]} onChange={value=>updateData(day.day,field.key,value)}/>)}</section>)}<div className="wb-day-actions"><button type="button" className="wb-secondary" disabled={day.day===1} onClick={()=>setSelectedDay(day.day-1)}>← 上一天</button><button type="button" className="wb-save-button" onClick={()=>saveDay(day.day,days[day.day]?.data??{})}>儲存今天紀錄</button><button type="button" className="wb-complete" onClick={()=>completeDay(day.day)}>{days[day.day]?.isCompleted?"✓ 已完成":"完成 Day "+day.day}</button><button type="button" className="wb-secondary" disabled={day.day===7} onClick={()=>setSelectedDay(day.day+1)}>下一天 →</button></div></main>}</div>;
}
