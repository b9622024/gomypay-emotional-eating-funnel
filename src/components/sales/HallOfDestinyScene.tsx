...
export default function HallOfDestinyScene() {
  ...
  const desktopExitWhiteOpacity = pulse(progress, 0.93, 0.965, 0.995) * 0.88;
  const exitWhiteOpacity = isDesktopScrollMode ? desktopExitWhiteOpacity : smooth(segment(progress, 0.955, 1));
  const cameraShift = 0;
    ...
    >
      ...
      >
        ...
        {activeGroupCopyOpacity > 0.02 && progress < 0.9 && (
          <div
            className={`hall-group-copy is-${activeGroupCopy.key}`}
            style={{
