...
export default async function ThankYou({params}:{params:Promise<{orderNo:string}>}){
  ...
  return <main className="success-page"><div className="container">
    <section className="success-card"><SuccessMark/><span className="eyebrow">付款完成</span><h1 className="title">恭喜你完成購買</h1><p>{aiOnly?"你的「AI 能量減脂初評」已經準備好了。":"你的《下班後嘴饞止損包》已經準備好了。"}</p><p className="muted">{aiOnly?"請點擊下方 LINE 官方帳號與我們聯繫，我們會提供測驗並安排後續一對一解析。":"請先完成第 1 天的情緒性進食 6 型測驗，接著開始記錄今天最容易嘴饞的時間點。"}</p>{!aiOnly&&<div className="next-steps"><article><span>STEP 1</span><p>打開工具包</p></article><article><span>STEP 2</span><p>完成 6 型測驗</p></article><article><span>STEP 3</span><p>開始今天的記錄</p></article></div>}<a className="btn" href={aiOnly?"https://lin.ee/UKTsrwq":`/access/${order.entitlements[0].accessToken}`} target={aiOnly?"_blank":undefined} rel={aiOnly?"noreferrer":undefined}><span>{aiOnly?"聯絡我們，領取 AI 能量減脂測驗":"前往我的工具包"}</span><span>→</span></a><div className="contact-links">{c.contacts.map(item=><a href={item.href} target="_blank" rel="noreferrer" key={item.label}>{item.label} ↗</a>)}</div><p className="muted" style={{fontSize:12}}>使用上有任何問題，都可以透過以上官方社群聯絡我們。</p></section>
