...
export default async function PersonalRescueMapPage({ params }: { params: Promise<{ accessToken:string }> }) {
  ...
  return <main className="bt-app"><header><Link href={`/access/${accessToken}`}>← 回到計畫首頁</Link><b>{state.progress.actionPoints} 行動點數</b></header><div className="bt-overview"><RescueMap token={accessToken} map={state.map} points={state.progress.actionPoints}/><BadgeCollection earnedBadges={state.progress.earnedBadges.map(String)} entries={state.entries.map(entry=>({earnedBadge:entry.earnedBadge,completedAt:entry.completedAt,actionPointsEarned:entry.actionPointsEarned}))} characterCreated={state.progress.characterCreated}/><ReplayPlanButton token={accessToken}/></div></main>;
}
