...
export default async function Access({params}:{params:Promise<{accessToken:string}>}){
  ...
  return <main className="access-page"><div className="container">
    ...
    <section className="access-section"><div className="access-section-heading"><span>RECOMMENDED ORDER</span><h2>建議使用順序</h2><p>不用一次全部做完，照著順序慢慢開始就好。</p></div><div className="progress-strip access-four-steps">{recommendedUsage.map(item=><article key={item.step}><span>STEP {item.step}</span><h3>{item.title}</h3><p>{item.description}</p>{item.action==="quiz"&&ownsMain&&<a href={`/access/${accessToken}/quiz/emotional-eating`}>開始測驗 →</a>}{item.action==="three-minute"&&ownsMain&&<a href={`/access/${accessToken}/three-minute-check`}>開始 3 分鐘檢查 →</a>}{item.action==="workbook"&&ownsMain&&<a href={`/access/${accessToken}/workbook`}>開始七日紀錄 →</a>}</article>)}</div></section>

    <section className="access-section"><div className="access-section-heading"><span>YOUR PRODUCTS</span><h2>已購買產品</h2><p>以下內容只根據這筆訂單實際購買的商品顯示。</p></div>{assets.length?<div className="access-grid">{assets.map((item,index)=><article className="access-item" key={item.key}><div className="asset-meta"><span className="tool-number">TOOL {String(index+1).padStart(2,"0")}</span><small>{item.kind}</small></div><h2>{item.title}</h2><p>{item.description}</p>{item.key==="emotional-eating-quiz"?<div className="asset-actions"><a className="btn" href={`/access/${accessToken}/quiz/emotional-eating`}><span>開始手機互動測驗</span><span>→</span></a><PdfButton href={emotionalEatingDownloadLinks.quizGuidePdf} label="下載 PDF 解讀手冊"/></div>:item.key==="three-minute-workbook"?<a className="btn asset-three-minute" href={`/access/${accessToken}/three-minute-check`}><span>開始 3 分鐘檢查</span><span>→</span></a>:item.key==="reset-action-guide"?<div className="asset-actions"><PdfButton href={emotionalEatingDownloadLinks.actionGuidePdf} label="下載 PDF 閱讀版"/><a className="btn asset-interactive" href={`/access/${accessToken}/workbook`}><span>開啟手機互動版</span><span>→</span></a></div>:item.key==="ai-energy-assessment"?<a className="btn asset-ai" href={`/ai-energy-assessment?accessToken=${encodeURIComponent(accessToken)}`}><span>了解領取與人工解析流程</span><span>→</span></a>:item.downloadUrl==="#"?<span className="tool-state">連結準備中 · 之後可替換為 Drive／Notion／Tally</span>:<a className="btn" href={item.downloadUrl} rel="noopener noreferrer"><span>開啟內容</span><span>→</span></a>}</article>)}</div>:<div className="access-empty">目前沒有可顯示的數位產品，請聯絡客服協助確認。</div>}</section>
